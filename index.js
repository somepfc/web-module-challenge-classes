...
class Car {
  ...
  drive(distance){
   let juice= this.tank * this.milesPerGallon
    if (juice >= distance){
    this.odometer = this.odometer + distance;
    this.tank = this.tank - (distance / this.milesPerGallon);
    } else {
      this.odometer= this.odometer + juice;
      this.tank = 0
...
*/
class studentextnd extends Student{
...
graduate(){
  if (this.grade >= 70){
    ...
    return "go back to grading their assignments to increase their score, so they can graduate"
  }
}
}
//End of Challenge
